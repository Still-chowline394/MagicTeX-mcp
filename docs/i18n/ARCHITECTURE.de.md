...
- `src/git/historyRepo.ts` — entscheidet, wo der Verlauf eines Projekts liegt. Ein Git-Repository behält ihn auf der versteckten Ref in sich selbst; ein einfacher Ordner bekommt ein eigenes Repository von uns unter `.latex-preview/history.git`, mit dem Projekt als Arbeitsbaum — der Verlauf folgt also dem Paper statt dem Pfad: Er wandert, kopiert und verschwindet mit dem Ordner, und `git` meldet dort weiterhin kein Repository. Verläufe von vor 0.1.9 lagen im Nutzer-Cache unter einem Hash des Pfads; einer wird nur übernommen, wenn seine aufgezeichneten Bytes noch zu einer Datei auf der Platte passen — ein wiederverwendeter Pfad kann die Checkpoints eines anderen Projekts also nicht erben.
