...
- `src/git/historyRepo.ts` — decide onde mora o histórico de um projeto. Um repositório git o mantém na ref oculta dentro de si mesmo; uma pasta comum ganha um repositório nosso em `.latex-preview/history.git`, com o projeto como árvore de trabalho — assim o histórico segue o artigo e não o caminho: se move, se copia e é apagado junto com a pasta, e `git` rodado ali continua dizendo que não há repositório. Históricos anteriores a 0.1.9 ficavam no cache por usuário sob um hash do caminho; um só é trazido adiante quando os bytes que ele registrou ainda batem com um arquivo em disco, de modo que um caminho reutilizado não pode herdar os checkpoints de outro projeto.
